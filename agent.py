...
warnings.filterwarnings(
    "ignore", category=RuntimeWarning, module="phi.tools.duckduckgo"
)
...
if not GROQ_API_KEY:
    raise ValueError(
        "GROQ_API_KEY environment variable is required. Set it in Railway or your .env file"
    )
...
async def run_agent_team():
    ...
    while True:
        try:
            ...
            news_res = researcher.run(
                "Search Arxiv for the top 3 most significant AI research papers from the last 7 days. Summarize the best one based on its abstract."
            )
            state["news"] = news_res.content

            # Helper to extract link
            link_match = re.search(
                r"http[s]?://arxiv\.org/pdf/[\w\.]+", news_res.content
            )
            if not link_match:
                link_match = re.search(
                    r"http[s]?://arxiv\.org/abs/[\w\.]+", news_res.content
                )
            ...
            analogy_res = teacher.run(
                f"Explain this research using an analogy: {news_res.content}"
            )
            state["analogy"] = analogy_res.content

            # Step 3: Design
            state["status"] = "🎨 Agent 3: Aligning Notification..."
            notif_res = designer.run(
                f"Create a notification based on this Analogy: '{analogy_res.content}'. \n\n But ensure it is grounded in this Real Research: '{news_res.content}'"
            )
